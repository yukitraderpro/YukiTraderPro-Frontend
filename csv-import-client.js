/* ==========================================================================
   Yuki Trader Pro — Assistant d'import CSV (V3.2, additif)
   --------------------------------------------------------------------------
   MODULE INDÉPENDANT DU MOTEUR D'ANALYSE. Toute la logique de parsing,
   mapping, déduplication et suppression vit côté backend
   (voir backend/src/services/csvImportService.js) — ce fichier ne fait que
   lire le fichier choisi par l'utilisateur, appeler l'API, et afficher les
   résultats. Fonctionne uniquement en mode serveur (isolation par
   utilisateur garantie côté backend, jamais côté client).
   ========================================================================== */
(function () {
  ...
  let currentPreview = null; // { importId, headers, sampleRows, suggestedMapping }
  ...
  async function handleUpload() {
    ...
    try {
      ...
      status.textContent = e.message; // déjà un message clair renvoyé par le backend, jamais de code brut
    }
  }
  ...
  /* ---- Historique des imports + suppression/restauration -------------------- */
