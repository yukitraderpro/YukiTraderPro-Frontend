/* ==========================================================================
   Yuki Trader Pro — js/yuki-knowledge.js
   --------------------------------------------------------------------------
   MODULE INDÉPENDANT, SANS IMPACT SUR LE MOTEUR D'ANALYSE (voir le cahier
   des charges « Assistant Yuki »). Ce fichier ne contient QUE des
   explications pédagogiques sur l'utilisation de l'application et la
   signification des termes déjà affichés à l'écran — jamais une
   recommandation de trading personnalisée, jamais une invention.

   Bilingue (FR/EN) : chaque entrée a un bloc `fr` et un bloc `en`
   (mots-clés, question, réponse). `findBestAnswer(query, screen, lang)`
   ne cherche que dans la langue demandée, pour ne jamais répondre en
   français à une question posée en anglais (ou l'inverse).

   Principe de sécurité fondamental : ce chatbot ne génère AUCUNE réponse.
   Il recherche, par correspondance de mots-clés, l'entrée la plus proche
   dans cette base de connaissances FIXE et VALIDÉE ci-dessous, et renvoie
   son texte tel quel. S'il ne trouve rien d'assez pertinent, il le dit
   explicitement et propose de contacter le support — il ne complète
   jamais un trou avec une réponse plausible mais non vérifiée.

   Pour modifier une réponse ou en ajouter une : voir le guide
   administrateur (GUIDE_ADMIN_ASSISTANT_YUKI.md) — aucune connaissance de
   programmation n'est nécessaire pour éditer le texte d'une entrée
   existante.
   ========================================================================== */
...
/* Score de correspondance simple par mots-clés partagés (pas de LLM, pas de
   requête réseau) : suffisant pour un FAQ borné, et garantit qu'aucune
   réponse n'est « inventée » — soit une entrée valide correspond, soit on
   le dit clairement. */
...
function scoreEntry(entry, normalizedQuery, currentScreen, lang) {
  ...
  for (const kw of loc.keywords) {
    ...
    if (kwWords.length === 1) {
      if (kwWords[0].length >= 2 && queryWords.has(kwWords[0])) score += 1; // mot isolé : correspondance exacte, jamais de sous-chaîne
    } else if (normalizedQuery.includes(nkw)) {
      score += kwWords.length; // expression multi-mots : les correspondances plus longues et plus spécifiques comptent davantage
    }
  }
  if (score > 0 && currentScreen && entry.screens.includes(currentScreen)) score += 0.5; // léger bonus contextuel
  return score;
}

/* Renvoie la meilleure entrée (avec question/answer déjà dans la bonne
   langue) si elle dépasse un seuil de confiance minimal, sinon `null` —
   c'est ce `null` qui déclenche la proposition de contacter le support
...
/* Suggestions de questions rapides pour l'écran actif, dans la langue demandée. */
