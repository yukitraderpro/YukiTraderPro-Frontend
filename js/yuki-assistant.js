/* ==========================================================================
   Yuki Trader Pro — js/yuki-assistant.js
   --------------------------------------------------------------------------
   Orchestration du chatbot Yuki (mascotte + assistant d'analyse). Ce module
   ne modifie JAMAIS le moteur d'analyse (analysis.js) : il consomme
   uniquement des résultats déjà produits (via buildSimpleAiBrief /
   buildCopilotBrief, deux fonctions de PRÉSENTATION pures déjà exposées par
   analysis.js, aucun recalcul de score) et le texte validé de
   js/yuki-messages.js + js/yuki-knowledge.js.

   Entièrement bilingue (FR/EN) : chaque texte affiché — chrome statique du
   widget (bouton, placeholder, boutons rapides…) ET contenu dynamique
   (réponses de la base de connaissances, explications d'analyse, rapport
   support) — suit la langue active de l'application (`currentLang()`,
   définie dans auth.js). `refreshContext()` est appelée par
   `window.refreshDynamicI18n` (app.js) à chaque changement de langue pour
   retraduire immédiatement tout ce qui est déjà affiché.

   N'appelle jamais l'API Twelve Data, ne touche jamais `state.apiKey`
   autrement que pour vérifier sa PRÉSENCE (jamais sa valeur), et n'a aucun
   accès ni influence sur analysis.js.

   Contrat fonctionnel exposé (cahier des charges V3.3, section 13) :
...
(function (root) {
  ...
  /* Cache-busting : le 19/07, le détourage de la mascotte a été refait
     (BUG UI-014-bis, oreille droite auparavant amputée) en réécrivant les
     MÊMES noms de fichiers avec de nouveaux octets. Sans ce paramètre de
     version, le cache HTTP du navigateur (et/ou du CDN d'hébergement, en
     plus du cache du service worker) peut continuer à servir indéfiniment
     l'ancienne image sous cette URL, même après la correction côté
     serveur — c'est très probablement la cause d'un rognage encore visible
     malgré un fichier source désormais correct. À incrémenter à chaque
     nouvelle modification des fichiers image de la mascotte. */
  /* Cache-busting : voir RAPPORT_CHANGEMENTS_UI015_AVATAR_DEFINITIF.md.
     À incrémenter à CHAQUE modification des fichiers image de la
     mascotte, sans exception — c'est le seul moyen fiable d'éviter qu'un
     cache HTTP/CDN ne serve indéfiniment une ancienne version sous la
     même URL. */
  ...
  /* Retraduit tout le chrome statique du widget (créé une seule fois par
     ensureWidget, donc jamais retouché par le système générique data-i18n
     de l'application) — appelée à l'initialisation ET à chaque changement
     de langue via refreshContext(). C'est précisément ce qui manquait :
     sans cet appel, seul le reste de l'application changeait de langue,
     jamais l'onglet « Besoin d'aide ? ». */
  function retranslateChrome() {
    const fab = document.getElementById("assistantFab");
    if (!fab) return; // widget pas encore créé, rien à retraduire
  ...
  function explainAnalysis(analysisResult, displayMode) {
    ...
    // buildSimpleAiBrief/buildCopilotBrief terminent déjà, la plupart du
    // temps, par un rappel que la décision finale revient à l'utilisateur —
    // on n'ajoute la clôture de la bibliothèque de messages que si ce
    // rappel n'est pas déjà présent, pour éviter un message répétitif.
  ...
  /* Appelée par window.refreshDynamicI18n (app.js) à chaque changement de
     langue, ET à chaque ouverture/rafraîchissement du panneau : retraduit
     le chrome statique, et redessine les réponses rapides + le message
     d'accueil (si aucun historique) dans la langue active. Les messages
     déjà envoyés dans l'historique ne sont pas rétroactivement traduits
     (comportement identique à celui du reste de l'application). */
  function refreshContext() {
    retranslateChrome();
    const panel = document.getElementById("assistantPanel");
    if (panel && !panel.classList.contains("hidden-card")) {
      renderQuickReplies();
      if (!history().length) renderMessages(); // réaffiche la bulle de bienvenue dans la nouvelle langue
    }
  }
  ...
  /* Exports Node : uniquement les fonctions pures, sans dépendance DOM, pour
     permettre des tests automatisés rapides (voir tests/yuki-assistant.test.js)
     sans nécessiter de navigateur. Le comportement en navigateur ci-dessus
     est strictement inchangé par cet export. */
