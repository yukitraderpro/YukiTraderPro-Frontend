...
if(log && log.signal===r.signal && (now-log.time)<cooldownMs) return;
...
if(state.apiTechnicalLog.length>200)state.apiTechnicalLog.length=200;
save();
}
...
if(!state.positions.some(p=>p.uid===pos.uid))return;
...
const realIsFresh=real&&real.checkedAt&&(Date.now()-real.checkedAt)<10*60*1000;
if(realIsFresh){
$("apiUsageMinute").textContent="—";
...
const now=new Date();
if(now.getDay()!==0&&now.getDay()!==1)return;
...
if(obDlg)obDlg.addEventListener("cancel",e=>e.preventDefault());
...
productId=founder?"yuki_pro_founder_monthly":"yuki_pro_standard_monthly";
