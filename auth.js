/* ==========================================================================
   Yuki Trader Pro — Module Comptes / Abonnement / Langue (auth.js)
   --------------------------------------------------------------------------
   V4 « version commerciale » (cahier des charges V4.0, Partie 1) : le mode
   local a été intégralement supprimé. Ce module exige désormais un backend
   configuré (`window.YUKI_API_BASE`) et n'a plus JAMAIS de repli local :
     - aucune donnée de compte, de rôle ou d'abonnement en localStorage ;
     - le rôle, le niveau d'abonnement et la date d'expiration viennent
       exclusivement des réponses du backend (jamais calculés ici) ;
     - l'access token JWT ne vit qu'en mémoire (variable JS, perdu à chaque
       rechargement de page — c'est voulu) ;
     - le refresh token n'est ni stocké ni même lisible par ce script : il
       vit dans un cookie HttpOnly posé par le serveur (voir
       backend/src/routes/auth.js), rejoué automatiquement par le
       navigateur sur les appels à /api/auth/refresh et /api/auth/logout.
   Si le backend est injoignable (ou non configuré), l'application affiche
   "Impossible de contacter le serveur." et ne revient JAMAIS à un mode
   dégradé local — voir boot()/showFatalBackendError() plus bas.
   ========================================================================== */

const DEVICE_ID_KEY = "yuki_pro_device_id_v1";

/* Base d'API — obligatoire en version commerciale (plus de mode local). */
...
/* ---- Session (mémoire uniquement, jamais persistée) ---------------------- */
let SESSION = { user:null, accessToken:null };

/* Erreur distincte pour "serveur injoignable" (panne réseau/backend down),
   à ne jamais confondre avec "pas encore connecté" (401 normal). C'est le
   seul cas où l'UI doit afficher le message fatal exigé par le cahier des
   charges V4 (Partie 1.1). */
...
/* Appelle l'API backend en attachant l'access token en mémoire. Sur 401,
   tente UNE fois un rafraîchissement silencieux via le cookie HttpOnly
   (jamais manipulé directement ici : `credentials:"include"` suffit à ce
   que le navigateur le rejoue) avant de réessayer la requête d'origine. */
...
/* Rafraîchit l'access token via le cookie de refresh (silencieux : ne lève
   jamais, renvoie simplement true/false). Utilisé au boot pour restaurer
   une session existante sans redemander le mot de passe, et par apiFetch
   pour renouveler un access token expiré. */
...
/* Tente de restaurer une session au chargement de la page. Renvoie
   l'utilisateur si une session valide existe (cookie de refresh présent et
   valide), sinon `null` (cas normal : personne n'est connecté — ce n'est
   PAS une erreur). Lève BackendUnreachableError si le serveur est
   injoignable (panne réseau distincte d'une absence de session). */
...
/* Recharge l'utilisateur courant depuis le backend (source de vérité pour
   rôle/abonnement/expiration — jamais recalculé côté client). À appeler
   après toute action susceptible de changer ces informations (ex. retour
   d'écran de facturation Google Play). */
...
const I18N = {
  fr:{
    ...
    stopLabel:"Borne basse de volatilité (2× ATR)",
    targetLabel:"Borne haute de volatilité (4× ATR)",
    ...
    entrySuggestedLabel:"Prix actuel",
    levelsDisclaimer:"Ces bornes sont des multiples de l'ATR (volatilité mesurée), pas des niveaux d'entrée ou de sortie recommandés.",
    target1Label:"Borne haute 1 (1,6× ATR)",
    target2Label:"Borne haute 2 (2,6× ATR)",
    ...
    shareWeekBtn:"📤 Partager ma semaine",
    deleteAccountBtn:"Supprimer définitivement mon compte",
    deleteAccountDesc:"La suppression de votre compte efface définitivement vos analyses, positions, journal, réglages et clé API. Cette action est irréversible.",
    msgConfirmDeleteAccount:"Supprimer définitivement votre compte ? Toutes vos données seront effacées et cette action est IRRÉVERSIBLE.",
    msgDeleteAccountPassword:"Pour confirmer, saisissez votre mot de passe :",
    msgAccountDeleted:"Votre compte et vos données ont été supprimés.",
    quickScanNote:"Pré-sélection rapide : ce classement n'inclut ni la confirmation multi-unités de temps ni la corrélation. Ouvre un instrument pour l'analyse complète, qui fait foi.",
    ob4RiskTitle:"Quel est ton rapport au risque ?",
    ob4RiskBody:"Ça règle les seuils d'alerte par défaut — pas le moteur d'analyse, qui reste identique pour tout le monde. Modifiable à tout moment dans Réglages.",
    obRiskPrudent:"🛡️ Prudent",obRiskPrudentDesc:"Je veux peu d'alertes, uniquement les configurations les plus solides.",
    obRiskBalanced:"⚖️ Équilibré",obRiskBalancedDesc:"Les réglages recommandés par défaut.",
    obRiskDynamic:"⚡ Dynamique",obRiskDynamicDesc:"Je préfère voir plus d'opportunités, quitte à filtrer moi-même.",
    settingsProfileLabel:"Profil de trading",settingsRiskLabel:"Rapport au risque",
    testPushBtn:"📲 Tester la notification app fermée",
    testPushSent:"Notification envoyée ! Ferme l'app : elle doit arriver quand même. (Si erreur : Firebase pas encore configuré, voir GUIDE_ACTIVATION_PUSH.md)",
    ...
    /* ---- Raisonnement / motifs (moteur local app.js) ---- */
    ...
    /* ---- Cartes positions / scalping / journal (chaînes composées) ---- */
    ...
    /* ---- V3.4 : parcours d'accueil (onboarding) ---- */
    ...
    deletionPageLink:"Suppression du compte et des données",
    legalTitle:"Confidentialité et documents légaux",
    legalDesc:"Consultez à tout moment la politique de confidentialité et la procédure de suppression de compte.",
  ...
  en:{
    ...
    stopLabel:"Lower volatility band (2× ATR)",
    targetLabel:"Upper volatility band (4× ATR)",
    ...
    scalpingLabel:"SCALPING",
    entrySuggestedLabel:"Current price",
    levelsDisclaimer:"These bands are multiples of the ATR (measured volatility), not recommended entry or exit levels.",
    target1Label:"Upper band 1 (1.6× ATR)",
    target2Label:"Upper band 2 (2.6× ATR)",
    ...
    shareWeekBtn:"📤 Share my week",
    deleteAccountBtn:"Permanently delete my account",
    deleteAccountDesc:"Deleting your account permanently erases your analyses, positions, journal, settings and API key. This action cannot be undone.",
    msgConfirmDeleteAccount:"Permanently delete your account? All your data will be erased and this action is IRREVERSIBLE.",
    msgDeleteAccountPassword:"To confirm, enter your password:",
    msgAccountDeleted:"Your account and data have been deleted.",
    quickScanNote:"Quick pre-selection: this ranking excludes multi-timeframe confirmation and correlation. Open an instrument for the full analysis, which prevails.",
    ob4RiskTitle:"What's your relationship with risk?",
    ob4RiskBody:"This sets your default alert thresholds — not the analysis engine, which stays identical for everyone. Editable anytime in Settings.",
    obRiskPrudent:"🛡️ Cautious",obRiskPrudentDesc:"I want few alerts, only the most solid setups.",
    obRiskBalanced:"⚖️ Balanced",obRiskBalancedDesc:"The recommended default settings.",
    obRiskDynamic:"⚡ Dynamic",obRiskDynamicDesc:"I prefer seeing more opportunities, even if I filter myself.",
    settingsProfileLabel:"Trading profile",settingsRiskLabel:"Risk appetite",
    testPushBtn:"📲 Test the closed-app notification",
    testPushSent:"Notification sent! Close the app: it should still arrive. (If error: Firebase not configured yet, see GUIDE_ACTIVATION_PUSH.md)",
    ...
    deletionPageLink:"Account and data deletion",
    legalTitle:"Privacy and legal documents",
    legalDesc:"Access the privacy policy and the account deletion procedure at any time.",
...
function applyI18n(){
  ...
  /* Rafraîchit tout le contenu généré dynamiquement par app.js (cartes,
     libellés de signal/qualité/risque, messages d'état…) pour que le
     changement de langue soit total et instantané, pas seulement sur les
     libellés statiques marqués data-i18n. */
  if(typeof window.refreshDynamicI18n === "function") window.refreshDynamicI18n();
}

/* ==========================================================================
   Rendu de l'écran d'authentification et de la passerelle d'accès
   ========================================================================== */
...
function wireAuthScreen(){
  ...
  document.getElementById("authMode-signup").onsubmit = async e=>{
    ...
    try{
      ...
      await signUp(email,pw); // authentifie déjà (accessToken + cookie refresh posés par le backend)
      enterApp();
    }catch(err){ document.getElementById("authError").textContent = err.message; }
  };
...
/* ---- Badge « Membre Fondateur » (addendum V3.3) --------------------------
   Le statut Fondateur n'est jamais déduit côté client : on interroge le
   backend (source de vérité de l'offre verrouillée par utilisateur, voir
   subscriptionOffersService.js) plutôt que de deviner à partir du rôle. */
...
/* ---- Tarification dynamique (addendum V3.1 « Abonnements ») ------------
   Aucun prix n'est jamais codé en dur : en mode serveur, on affiche le
   tarif réellement en vigueur (et les places restantes pour l'offre
   Fondateur). En mode local (pas de backend configuré), il n'existe tout
   simplement aucune offre réelle à afficher — on le dit clairement plutôt
...
async function renderOfferPricing(){
  ...
  try{
    ...
    /* Présentation enrichie demandée par l'addendum V3.3 pour l'offre
       Fondateur — tous les chiffres viennent de `best`, jamais codés en
       dur (seule la mise en forme du texte est fixe). */
    if(founderCard){
