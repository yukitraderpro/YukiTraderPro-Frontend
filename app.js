...
/* Chaînes de repli par instrument. Le premier symbole disponible chez le
   fournisseur gagne. Les indices et matières premières « cash » ne sont pas
   couverts par tous les plans Twelve Data : on prévoit donc systématiquement
   un ETF liquide en proxy, dont la dynamique intraday suit le sous-jacent.
   Un instrument sans entrée ici retombe sur le symbole du catalogue, sans
   repli — c'est ce qui produisait les « données indisponibles ». */
...
/* Partage « Wrapped » : carte image générée en local (js/share-card.js).
   Accessible depuis le récap hebdo ET depuis le journal (bouton permanent),
   pour ne pas dépendre du dimanche. */
...
/* Suppression de compte (RGPD + exigence Google Play 2024). Double garde-fou :
   confirmation explicite, puis ressaisie du mot de passe — la suppression est
   irréversible et ne doit pas pouvoir résulter d'un tap accidentel ou d'une
   session laissée ouverte. Les données locales sont effacées ensuite, sinon
   l'appareil garderait une copie des analyses supprimées côté serveur. */
